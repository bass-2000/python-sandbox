...
for name_goods, label in goods.items():

    total_item_price = 0

    for code, list_store in store.items():
        index = 0
        for ls in list_store:
            index += 1
            if code == label:
                ss = ls['quantity'] * ls['price']
                print(f'На складе товаров: {name_goods} {index} типа на сумму {ss} рублей')
